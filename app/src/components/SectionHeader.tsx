...
export default function SectionHeader({ title, subtitle, glowColor = 'pink', className = '' }: SectionHeaderProps) {
  ...
  return (
    <div ref={ref} className={`text-center mb-12 md:mb-16 ${className}`}>
      <h2 className="font-orbitron font-bold text-xl md:text-2xl lg:text-3xl text-navy tracking-[0.08em] mb-3">
        {title}
      </h2>
      <p className={`font-orbitron font-bold text-sm md:text-base lg:text-lg tracking-[0.15em] ${textClasses[glowColor]}`}>
        {subtitle}
      </p>
    </div>
  );
